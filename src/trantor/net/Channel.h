 ...
 *
 */
class ZLTOOLKIT_EXPORT Channel : toolkit::noncopyable
{
  public:
     ...
     */
    Channel(const std::shared_ptr<toolkit::EventPoller> &loop, int fd);

    /**
     * @brief Destroy the Channel instance.
     *
     * Cleanly removes the channel from the poller.
     */
    ~Channel();
     ...
     *
     * @return std::shared_ptr<toolkit::EventPoller>
     */
    std::shared_ptr<toolkit::EventPoller> ownerLoop()
    {
        return loop_;
    };
  ...
  private:
    ...
    // Convert POLL* mask to EventPoller event mask
    int toPollerEvents(int pollEvents) const;

    // Convert EventPoller event mask to POLL* mask
    int fromPollerEvents(int pollerEvents) const;

    void update();
    void handleEvent();
    void handleEventSafely();

    // Called by EventPoller callback; sets revents_ and invokes handlers
    void onPollerEvent(int pollerEvents);

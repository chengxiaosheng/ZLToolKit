...
{
    TraceL << conn->localAddr().toIpPort() << " -> "
...
TcpClient::TcpClient(const std::shared_ptr<toolkit::EventPoller> &loop,
...
{
    (void)validateCert_;
    TraceL << "TcpClient::TcpClient[" << name_ << "] - connector ";
}
...
TcpClient::~TcpClient()
{
    ...
    loop_->async([conn = std::move(conn)]() {
        conn->setCloseCallback([](const TcpConnectionPtr &connPtr) mutable {
            connPtr->getLoop()->async(
                [connPtr] { connPtr->connectDestroyed(); }, false);
        });
    });
...
{
    // TODO: check state
    TraceL << "TcpClient::connect[" << name_ << "] - connecting to "
...
void TcpClient::newConnection(int sockfd)
{
    ...
    TraceL << "SSL enabled: " << (tlsPolicyPtr_ ? "true" : "false");
    if (tlsPolicyPtr_)
    {
    ...
    auto closeCb = std::function<void(const TcpConnectionPtr &)>(
        [weakSelf](const TcpConnectionPtr &c) {
            ...
            {
                TraceL << "TcpClient::removeConnection was skipped because "
...
void TcpClient::removeConnection(const TcpConnectionPtr &conn)
{
    ...
    loop_->async([conn]() { conn->connectDestroyed(); }, false);
    if (retry_ && connect_)
    {
        TraceL << "TcpClient::connect[" << name_ << "] - Reconnecting to "
                  << connector_->serverAddress().toIpPort();
        connector_->restart();
    }
}

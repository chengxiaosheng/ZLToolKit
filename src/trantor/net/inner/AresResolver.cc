...
std::shared_ptr<Resolver> Resolver::newResolver(const std::shared_ptr<toolkit::EventPoller> &loop,
                                                size_t timeout)
{
    return std::make_shared<AresResolver>(loop, timeout);
}

AresResolver::AresResolver(const std::shared_ptr<toolkit::EventPoller> &loop, size_t timeout)
                                 ...
                                 const ResolverResultsCallback& cb)
{
    ...
    {
        loop_->doDelayTask(timeout * 1000, [this, self = shared_from_this()]() {
            AresResolver::onTimer();
            return 0;
        });
        timerActive_ = true;
    }
...
void AresResolver::onTimer()
{
    ...
    {
        loop_->doDelayTask(timeout * 1000, [self = shared_from_this(), this]() {
            AresResolver::onTimer();
            return 0;
        });
    }
...
#endif
{
    TraceL << "sockfd=" << sockfd << " type=" << getSocketType(type);
...
void AresResolver::ares_sock_statecallback_(void* data,
                                            ...
                                            int write)
{
    TraceL << "sockfd=" << sockfd << " read=" << read << " write=" << write;
